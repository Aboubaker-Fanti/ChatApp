...
export default function Home() {
  ...
  return (
    ...
    <div className="flex h-[100%] rounded-3xl w-[100%] bg-gradient-to-b from-red-900 via-purple-900 to-blue-900">
      {/* Left Sidebar */}
      {/* <div className="w-16 bg-black/30 backdrop-blur-md p-4 flex flex-col items-center space-y-6">
        <div className="w-8 h-8 rounded-full bg-pink-200 overflow-hidden">
          <img src="/api/placeholder/32/32" alt="User" className="w-full h-full object-cover" />
        </div>
        {sidebarIcons.map((item, index) => (
          <button key={index} className="text-white/70 hover:text-white transition-colors">
            {item.icon}
          </button>
        ))}
      </div> */}
      ...
      {/* User List */}
      <div  className="w-1/2 rounded-3xl bg-black/20 backdrop-blur-md  p-4">
        <div className="mb-6">
          <input
            type="text"
            placeholder="Search"
            className="w-full px-4 py-2 rounded-lg bg-black/30 text-white placeholder-white/50 outline-none"
          />
        </div>
        <div  className="scrollbar-hidden space-y-2 overflow-y-scroll max-h-150 ">
          {users.map((user) => (
            <button
              key={user.id}
      ...
      {/* Profile Info */}
        <div className="w-full  py-8 rounded-3xl  backdrop-blur-md border-0">
          <div className="p-6">
            <h2 className="text-2xl w-full py-7 flex items-center justify-center" >Profile info</h2>
            <div className="flex flex-col items-center">
              <div className="w-35 h-35 rounded-full bg-pink-200 overflow-hidden mb-4">
                <img 
              ...
              <h2 className="text-xl py-1 font-semibold text-white mb-2">{selectedUser.name}</h2>
              <p className="text-white/70 text-center py-9 mb-4">{selectedUser.description}</p>
              <button className="w-[55%] p-4 cursor-pointer my-12 rounded-2xl bg-white/15 hover:bg-white/20 text-white">
                Send Message
              </button>
            </div>
          </div>
